    ...
    const fetchCategory = async () => {
      try {
        ...
        const response = await axios.get(
          `https://cosmetics-server-001.vercel.app/api/admin/skin-care/${categoryId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://cosmetics-server-001.vercel.app/api/admin/update/skin-care/${categoryId}`,
        formDataToSend,
        {
          headers: {
      ...
      Swal.fire("Success!", "brand updated successfully!", "success").then(
        () => {
          router.push("/admin/products/add-skincare");
        }
      );
    } catch (err) {
      setError(err.message);
...
export default EditSkinCarePage;
