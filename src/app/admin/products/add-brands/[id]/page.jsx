    ...
    const fetchCategory = async () => {
      try {
        ...
        const response = await axios.get(
          `https://cosmetics-server-001.vercel.app/api/admin/brand/${categoryId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://cosmetics-server-001.vercel.app/api/admin/update/brand/${categoryId}`,
        formDataToSend,
        {
          headers: {
      ...
      Swal.fire("Success!", "brand updated successfully!", "success").then(
        () => {
          router.push("/admin/products/add-brands");
        }
      );
    } catch (err) {
      setError(err.message);
